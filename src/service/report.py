...
def Main():
    ...
    for inc in ['INC1', 'INC2', 'INC3', 'INC4']:
        ...
        fig = plt.figure(figsize=(20, 12))  # Adjusted size for better layout
    ...
    for inc in ['INC1', 'INC2', 'INC3', 'INC4']:
        ...
        # Plotting
        fig = plt.figure(figsize=(18, 8))  # Adjusted size for better layout
        ...
        for row_id, group in a_axis_data.groupby('Row ID'):
            ax1.plot(group['Date'], group['Value'], marker='o', label=row_id)
        ...
        for row_id, group in b_axis_data.groupby('Row ID'):
            ax2.plot(group['Date'], group['Value'], marker='o', label=row_id)
    ...
    for source in data_sources:
        ...
        if response.status_code == 200:
            ...
            if all_no_pressure:
                ...
                table_data = pd.DataFrame(index=[0], columns=columns_to_plot)
